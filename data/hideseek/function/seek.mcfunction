...
execute if score #radarEnabled radarEnabled matches 1 run item replace entity @a[team=2] inventory.2 with nether_star[food={nutrition:0,saturation:0,can_always_eat:true,eat_seconds:0.05},custom_name='{"text":"Radar","color":"green","italic":false}'] 10
...
execute unless score kaelos Kaelos matches 1.. run execute unless score ataraxia Ataraxia matches 1.. run execute unless score tempus Tempus matches 1.. run execute unless score hoa HOA matches 1.. run execute unless score redstoneAcademy redstoneAcademy matches 1 run schedule function hideseek:border1 300s
