...
execute if score #bunnyTimer bunnyTimer matches 0 run effect give @e[type=rabbit,nbt={RabbitType:99}] jump_boost infinite 3 true

# Sounds
execute as @a at @s run playsound minecraft:entity.evoker.prepare_summon master @s

# Remove 1 from the timer
scoreboard players remove #bunnyTimer bunnyTimer 1

# Reschedule the function until the timer expires
execute unless score #bunnyTimer bunnyTimer matches 0 run schedule function hideseek:events/killerbunny/summon 1s